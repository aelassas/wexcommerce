...
export const getNotificationCounter = (context, userId) => (
    axios.get(`${Env.API_HOST}/api/notification-counter/${encodeURIComponent(userId)}`, { headers: UserService.authHeader(context) }).then(res => res.data)
)
...
export const getNotifications = (context, userId, page) => (
    axios.get(`${Env.API_HOST}/api/notifications/${encodeURIComponent(userId)}/${page}/${Env.PAGE_SIZE}`, { headers: UserService.authHeader(context) })
        .then(res => res.data)
)