...
describe('Test validateAccessToken', () => {
  it('should test validateAccessToken', async () => {
    ...
    // test failure
    expect(await helper.validateAccessToken('unknown' as wexcommerceTypes.SocialSignInType, 'token', email)).toBeFalsy()

    // test success (facebook)
    let payload = {}
    let token = await encryptJWT(payload)
    expect(await helper.validateAccessToken(wexcommerceTypes.SocialSignInType.Facebook, token, email)).toBeTruthy()

    // test success (apple)
    payload = { email }
    token = await encryptJWT(payload)
    expect(await helper.validateAccessToken(wexcommerceTypes.SocialSignInType.Apple, token, email)).toBeTruthy()
    ...
    await jest.isolateModulesAsync(async () => {
      ...
      payload = { data: { email } }
      token = await encryptJWT(payload)
      expect(await helper.validateAccessToken(wexcommerceTypes.SocialSignInType.Google, token, email)).toBeTruthy()
      jest.resetModules()
      jest.clearAllMocks()
    })
  })
